#!/bin/sh
# mkimage.sh OUTPUT.img BOOTX64.EFI kernel.elf
#
# Build a FAT32 disk image holding the UEFI loader at the removable-media
# fallback path and the kernel where the loader expects it. Uses mtools so
# no root or loop devices are needed on any host.
set -eu
