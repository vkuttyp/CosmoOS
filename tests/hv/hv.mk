...
HV_GUESTS := guest_wfi guest_hvc guest_mmio guest_sysreg guest_idreg guest_spi guest_spin guest_irq guest_timer guest_ctimer guest_gicd guest_gicc guest_gic guest_sgi guest_mmio_widths guest_uart guest_uart_rx guest_uart_wfi guest_uart_poll guest_ptimer guest_timer_wfi guest_offspin guest_psci_race
endif
...
# The virtio-net C guest that el2-virtq-net backs (reuses the block guest's
# flat entry: both just set a stack and call guest_main).
$(HV_TEST_OUT)/guest_vnet.bin: $(ROOT)/tests/hv/aarch64/guest_vblk_start.S $(ROOT)/tests/hv/aarch64/guest_vnet.c
	$(call log,CC,$@)
	$(Q)mkdir -p $(dir $@)
	$(Q)$(CC) --target=aarch64-unknown-none-elf -c $(ROOT)/tests/hv/aarch64/guest_vblk_start.S -o $@.start.o
	$(Q)$(CC) $(HV_CGUEST_CFLAGS) -c $(ROOT)/tests/hv/aarch64/guest_vnet.c -o $@.main.o
	$(call log,BIN,$@)
	$(Q)$(LD) --image-base=0 -Ttext=0x1000 --oformat=binary -o $@ $@.start.o $@.main.o
