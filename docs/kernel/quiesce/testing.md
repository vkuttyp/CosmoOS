...
- No test drives the straggler IPI (Q6) or the `blk_submit`/`blk_unregister`
  window (Q11); both are reviewed, not exercised. **Since the
  virtio-removal unit the second is exercised on real hardware too**:
  `virtio-remove-inflight` removes the machine's removal disk with the
  driver's slot table full and asserts what the removal completed, in
  what order, and that the function can be re-probed
  (`docs/kernel/device/testing.md`).
