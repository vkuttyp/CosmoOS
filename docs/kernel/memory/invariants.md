...
**M30. The near arena lies above the kernel image and within the
architecture's branch reach of it.** The bounds come from
`arch_mmu_near_arena()` (x86-64: `0xFFFFFFFF88000000` –
`0xFFFFFFFFFF000000`, the top 2 GiB, so code built with
`-mcmodel=kernel` addresses itself and the image with sign-extended
32-bit relocations; AArch64: `align2M(__kernel_end)` – image base +
120 MiB, so `CALL26` reaches every export); `vmm_init` panics if
`__kernel_end` exceeds the low bound. Checked by: assert (`vmm_init`),
test `module-load` (a module's exported function is called through a
`PC32`- or `CALL26`-relocated call and returns the right value), review.
