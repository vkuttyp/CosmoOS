...
| `sched-balance-pair` | the pair, made on purpose: two workers pinned to one CPU (not the test thread's), their premise **observed** (both have run their loop since their release -- each has written the pair's CPU into its `cpu`, which starts as `BAL_CPU_UNSEEN` rather than 0, because a zero read as CPU 0 and made a not-yet-run worker look separated; a spinning pair also *settled* -- neither queued without `PREEMPTED`, because a worker that has never run carries no mark and is movable, which one boot in nine found when the premise was only "one queued `PREEMPTED`"), then widened to every CPU. The yielding pair is separated within a bound; the spinning pair still shares one CPU at the end of the same bound -- S26 seen from outside, and the reason the pull test's workers yield. The halves differ in the yield alone, so each is the other's control. A premise not seen within 1 s is its own failure, distinct from either claim |
...
`bench-balance` reports its ratio and asserts nothing, because the
balancer has **one window**: it cannot move a thread that is
time-slicing, since the one in the queue always carries
`THREAD_FLAG_PREEMPTED` (S26). While the released threads are runnable
and have not yet run, an idle CPU can take one; once they are
alternating, the imbalance is permanent for that round. An AArch64 boot
measured the miss -- placement `0/2/1/1`, one pull, 62% of the balanced
