...
asked) -- one deadline, reported per call where it is armed
(`lockup_sample_all_info`: claims, waits, the interval), which is how the
test checks it without a clock (a guest's wall clock measures
