...
| `lockup-sample-busy` | two threads on CPUs 0 and 1 ask at the same moment: exactly one gets the slot; the loser gets `false` with an empty mask, and `samples_busy` counts it; **the loser returned while the winner still held the slot** (the winner holds it until it sees the loser return, 1 s guard) **with one claim attempt and no wait** (`lockup_sample_all_info`'s report of that call). With three CPUs, two targets spinning with interrupts masked and that sample sent without its NMI (`LOCKUP_SAMPLE_IPI_ONLY`, so neither can answer on x86-64 either, where the NMI answered and the claim went untested): **one wait armed, for exactly `LOCKUP_SAMPLE_TIMEOUT_NS`**, and neither target in the mask. The only time bounds left are 1 s hang guards (the lockup-bound unit, `docs/audit/next-subsystem-lockup-bound.md`: the old `el < 7 ms` failed ten times in eight days on host stalls of 88-241 ms) | a wait per target → two waits; the flag ignored → on x86-64 both targets answer by NMI; the loser waiting for the slot → both get it in turn; the loser refused only once the slot is free → the order; the loser retrying the claim before refusing → more than one claim; a deadline armed for ten times the timeout → the interval |
