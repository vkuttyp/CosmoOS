...
| `lockup-sample-busy` | two threads on CPUs 0 and 1 ask at the same moment: exactly one gets the slot; the loser gets `false` with an empty mask, and `samples_busy` counts it; **the loser returned while the winner still held the slot** -- the winner holds it until it sees the loser return (1 s guard), so "refused at once" is an order, not a time. With three CPUs, two targets spinning with interrupts masked and the sampler forced to the ordinary interrupt (`lockup_test_ipi_only`, so neither can answer on x86-64 either, where the NMI answered and the claim went untested): **`samples_waits` rose by exactly one** (read while the test still holds the exclusive slot) and neither target is in the mask. The only time bounds left are 1 s hang guards (the lockup-bound unit, `docs/audit/next-subsystem-lockup-bound.md`: the old `el < 7 ms` failed ten times in eight days on host stalls of 88-241 ms) | a wait per target → `samples_waits` rises by two; the knob ignored → on x86-64 both targets answer by NMI; the loser waiting for the slot → both get it in turn and the winner's guard expires first |
