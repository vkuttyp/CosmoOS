...
| `drivers/network/` | The `e1000e` module: an Intel 82574L driver, the first NIC here that is not virtio — legacy descriptor rings, one MSI-X vector, a transmit watchdog, no offloads until a benchmark says they pay; the check on the stack's claim not to depend on a device |
