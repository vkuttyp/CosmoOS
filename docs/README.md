...
| `drivers/usb/` | The `xhci` module (the USB core: bus, enumeration, transfers, class binding; and the xHCI controller driver: rings, contexts, commands, the event ring, root-hub ports, one worker per controller) and the `usb_storage` module (bulk-only mass storage as `sda`); the DMA-through-the-controller rule and the children-first removal order the device model needed |
