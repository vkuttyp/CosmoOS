...
**Seventy-six, to 2026-09-22**, across CI and this developer's machine, on
...
| this developer's machine, the percpu-migration build, **under the chaos migrator**, twice more | observed, one x86-64 and one AArch64 `make test-chaos` boot of the final tree on 2026-09-22 (`7b6b8b79` and the commit before): `connect 0 in 905 ms` / `connect 0 in 867 ms`, `sent 12 in 0 ms`, `recv -104`, `outstanding 12 then 12`, `segs_out +3` **`retransmits +0`** `rsts_in +1`; host side accepted, `0 byte(s)`, `[deadline]`. Four of thirteen chaos boots against none of the plain ones on this tree: the guest queued its twelve bytes and retransmitted them at most once in the ten seconds before the reset, where its retransmit timer should have fired three times. The harness's failure line now prints the connection's retransmit timer state, CPU and expiry and its work item's state on a short line of its own (`NETTEST: client state:`; the long line was being cut by an interleaved print), for the next sighting. Kept in the family's tally until that line says otherwise. |
...
Sixty-one entries, seventy-six occurrences -- and the table is the tally,
