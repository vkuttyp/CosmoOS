...
| `install NAME[=VERSION]` | Resolve the closure from the stored index (`NAME=VERSION` is the constraint `NAME = VERSION`), then for each package in dependency order: find `file` in a repository, check size and SHA-512 against the index, verify the signature, cross-check the manifest, install | `pkg: installing NAME-VERSION (N files)`; `pkg: NAME will be upgraded from A to B` when a constraint needs a newer installed package |
...
| `remove NAME` | Refuse when another installed package depends on it unless `-f`; unlink the manifest's files; `rmdir` the recorded directories deepest first; drop the record | `pkg: NAME: OTHER depends on it`; `pkg: removing NAME-VERSION (N files)` |
