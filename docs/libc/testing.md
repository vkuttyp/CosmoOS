...
| --- | --- | --- |
| `env-spawn-under-setenv` | a thread looping `spawnvp` while another grows the environment and a third churns the heap — the reader of `environ` that lives outside `stdlib.c`. It drives **all four** of `spawnvp_flags`'s exits, which is where the snapshot is freed: an absolute path, a bare name found on `PATH`, a bare name that resolves nowhere, and an absolute path that cannot run — that last one checks the **errno survives the `free`**, which is the only reason that arm saves and restores it. The first build spawned only `/bin/true`, so the PATH-search half — the loop holding the snapshot across repeated attempts, and its frees — ran in no test; review found that, and then found the errno arm still missing | passes: a regression test, not a proof (the window is a few instructions inside a call that then spends milliseconds creating a process) |
