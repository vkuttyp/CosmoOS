...
| `env-pointer-survives-overwrite` | holds the pointer `getenv` returned, overwrites the name, reuses the heap 400 times, and reads it again — the contract the deliberate leak buys, which no other case here would have missed | **freeing the replaced string**: every other case passes with the leak tidied away, and this one reads what the churn wrote |
