...
| `el2-guest-timer-ontime` | `aarch64/guest_timer_wfi.S` | the idle loop every guest kernel has: arm, `WFI`, be woken. The `WFI` run must hold for most of the ~15 ms the guest asked for (measured on the host's clock), and the handler's own `CNTVCT` minus its `CVAL` -- lateness, in the guest's ticks -- must be less than four times the interval asked for. It was "less than the interval" and failed on a correct kernel when the host held the vCPU past 15 ms between the deadline and the guest's read (2026-09-13); four times still separates the park's 1 ms slice from a coarse park by an order of magnitude, and the test is on the load-sensitive list (`docs/testing/flakes.md`). Measured: asked 15.6 ms, held 17 ms, 1.8 ms late |
...
| `tap` (net) | -- | a tap interface whose far end is userland, against the real stack: a frame transmitted out the tap is read back (`netif_transmit` -> `tap_recv`); an ARP request injected for the tap's own IP is answered by the stack out the tap, the reply carrying the tap's MAC and IP to the asker (`tap_inject` -> the worker -> `arp_input` -> `netif_transmit` -> `tap_recv`); and the transmit queue caps at `TAP_TXQ_MAX` rather than growing. Dropping the frame in `tap_inject` leaves the ARP unanswered and the test times out |
| `el2-tap-host` | `aarch64/guest_tap.c` | the guest-to-host path end to end: the test creates a real tap (10.0.4.0/24) and bridges the inline virtio-net model's transmit to it; the guest brings the device up and transmits a broadcast ARP request for the host's tap IP, whose `virtio_net_hdr` the bridge strips before `tap_inject`. The host stack answers on its worker, and the test polls `tap_recv` for an ARP reply from the tap's MAC and IP -- so the guest's frame crossed virtio-net, the bridge, and into the real stack; disabling the bridge's inject fails it |
...
| machine mode (userland) | `guest_dtb.bin` via `vmctl run --machine -c 2` in `rc.test` | the whole path through the real owner: `vmctl` builds the tree, loads by the header, answers PSCI, runs each vCPU **on its own thread**, and stops at `SYSTEM_OFF`. The harness (`run_boot_test.py`, `HVTEST_MARKERS`) requires each guest line -- the `dtb:` line, the version, `cpu1: up ctx=1234cafe`, `cpu_on 1 -> 0` and `vmctl: guest powered off` -- because the userland shell has no `grep` to check them with in `rc.test`. `cpu1: up` is the interesting one: this guest starts a CPU and powers the machine off two instructions later, so the line appears only if the secondary really is running by then. Under the round-robin this replaced, that took a fairness rule and a 64-turn hold on the power-off; with a thread per vCPU it takes a **`CPU_ON` that waits for its target's first run to have returned**, which is the only evidence of "powered on and executing" that userland has. Two builds got this wrong before it held: the first had no wait at all and lost the marker exactly as the round-robin once had; the second waited for the target's *thread* to reach its entry, which this machine passed every time and a loaded CI runner failed immediately -- a thread can exist without its guest having run. Turned into a local proof by standing a 40 ms sleep in for the loaded host: with the wait on the run the boot passes, with the wait on the thread it fails on exactly this marker. The marker's history is three mechanisms and one race |
| machine mode, a CPU that never yields (userland) | `guest_offspin.S` via `vmctl run --machine -c 2` in `rc.test` | the first CPU starts the second at a spin loop and asks for `SYSTEM_OFF` at once. The machine must still stop, and `rc.test` prints `HVTEST: offspin ok` -- the whole observable, this guest reading no tree and printing nothing. What makes it stop is now the **kick** (`SYS_vcpu_stop`) rather than a bounded hold: the sibling is made to leave its run instead of being waited out. The failure is a hang, so the detector is the boot test's 180-second deadline -- observed at 187 s when the shutdown path is broken, with every marker after this run missing |
| machine mode, a vCPU the guest never starts (userland) | `guest_offspin.S` via `vmctl run --machine -c 3` in `rc.test` | the same guest with a spare CPU: it brings up only CPU 1, so **CPU 2's thread is parked on its futex** when `SYSTEM_OFF` arrives. The kick reaches a thread inside `cosmo_vcpu_run` and nothing else, so shutdown must wake the parked ones too or the owner's live count never reaches zero. `HVTEST: parked ok` is the marker. Proved by keeping the `QUIT` store and removing only the wake: `HVTEST: offspin ok` still passes -- a running thread sees `QUIT` when it next looks -- and the boot dies on its deadline before `parked ok`, which is what localises the failure to the parked case. This test exists because **no other one leaves a thread parked at shutdown**: every other guest starts every vCPU it is given, so the wake was unexercised until it was added |
| machine mode, PSCI during shutdown (userland) | `guest_psci_race.S` via `vmctl run --machine -c 3` in `rc.test` | two CPUs ask for a third in a loop while the first powers the machine off; the machine must still stop (`HVTEST: psci-race ok`). **What it does not prove**, and the guest says so in its own header: replacing the owner's compare-and-swap with a plain store does not fail it, because after the first `CPU_ON` for a target succeeds every later one is answered `ALREADY_ON` and returns before reaching the store -- so a guest asking in a loop still gets one attempt per target at a window a few instructions wide. It tests the shutdown path with PSCI arriving concurrently, which is a different and smaller claim than the one its name suggests |
| two vCPUs at once (userland) | every `--machine -c 2` run | `vmctl` counts the threads inside `cosmo_vcpu_run` and prints the peak; `vmctl: peak concurrent vcpus: 2` is a required marker. This is the unit's whole claim made countable, and the round-robin it replaced could never have produced it: one thread ran every vCPU, so the peak could not exceed 1 however the ticks interleaved. A counter rather than the timestamps the report proposed -- a peak above one *is* two intervals intersecting, with no clock and no comparison pass to get wrong. Proved by serialising the runs behind one mutex, as one thread would: the peak is 1 and the marker is missing. That injection also hangs the boot, which demonstrates the other half of the rule -- a lock held across `cosmo_vcpu_run` lets a spinning guest hold it forever |
| the kick itself (kernel) | `hv-vcpu-stop` | a vCPU running `guest_spin` is stopped from another thread and must exit `COSMO_VM_EXIT_STOPPED` -- and must have done so *after* the kicker sent (the kicker is joined, then `t1 >= sent_ns`: a run that ended on its own returned before the kick; read the instant the run returned, the kicker's flag used to race its own last two lines, since it is stored after the stop that ends the run); a stop set *between* runs is taken by the next run **without entering the guest**, which the entry counter proves; and it is consumed, so the run after it enters normally. Proved by removing the take at the top of the run loop, which fails on the entry counter. Two things this test reports rather than asserts, both because a bug-proof said so: the kick's latency (~60 us), and whether the kicker found the guest entered at all -- that depends on how the two threads are placed, and runs have gone both ways on one build. **Removing the IPI entirely does not fail this test**: `guest_spin.S` says why, since every host timer tick is taken to EL2 and the sticky flag is noticed at the next one. Liveness comes from the tick plus the flag; the IPI buys promptness and independence from the host's tick policy |
...
and loaded there), `guest_offspin` (a second CPU that never yields, for the
owner's bound on a held power-off -- see the machine-mode row above) and
`guest_idreg` -- one per exit the EL2 switch
