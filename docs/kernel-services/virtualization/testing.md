...
| machine mode (userland) | `guest_dtb.bin` via `vmctl run --machine -c 2` in `rc.test` | the whole path through the real owner: `vmctl` builds the tree, loads by the header, answers PSCI, runs the second vCPU `CPU_ON` created in turn with the first, and stops at `SYSTEM_OFF` -- but not before that second vCPU has had a turn that ended on its own terms, which is what keeps `cpu1: up` from going missing on a loaded host. That fairness is proved by robbing the fresh vCPU of its first turn in the loop (one experimental patch, which is what a preemption before its first instruction amounts to) and running the boot test twice: with the rule the marker still appears and the harness passes; with the rule removed the harness fails on exactly `missing marker /^cpu1: up ctx=1234cafe$/`, the console showing `cpu_on 1 -> 0` followed straight by `guest powered off` -- the CI signature that prompted the fix, reproduced on demand. The boot harness (`run_boot_test.py`, `HVTEST_MARKERS`) requires each guest line -- the `dtb:` line, the version, `cpu1: up ctx=1234cafe`, `cpu_on 1 -> 0` and `vmctl: guest powered off` -- because the userland shell has no `grep` to check them with in `rc.test` |
| machine mode, a CPU that never yields (userland) | `guest_offspin.S` via `vmctl run --machine -c 2` in `rc.test` | the other half of that rule: the first CPU starts the second at a spin loop and asks for `SYSTEM_OFF` at once, so the owner holds the power-off for a sibling that will never yield. The hold is bounded in turns, so the machine still stops and `rc.test` prints `HVTEST: offspin ok` -- which is the whole observable, this guest reading no tree and printing nothing. The bound is what the test is for: run the last runnable vCPU untimed (as the owner did before the fix, when `nr_running` drops to one) and the spinning sibling never returns, `vmctl` hangs, and the boot test dies on its 180-second deadline with every marker after this run missing -- observed at 187 s. `HVTEST: offspin ok` is a required marker like the `guest_dtb` run's lines above, and for the same reason -- `rc.test` cannot check them itself |
