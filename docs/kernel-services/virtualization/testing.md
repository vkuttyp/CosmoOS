...
| `el2-guest-uart-race` | `aarch64/guest_uart_poll.S` | a property about two threads, with three: a kernel thread types 300 bytes at a guest whose two vCPUs, each run by its own thread, both poll `DR` and both can take SPI 33 -- so the owner's raise, either vCPU's lower and either vCPU's per-entry re-raise race in the UART for every byte. No interrupt may arrive with `MIS` zero on either vCPU (the spurious one a raise decided from a stale state leaves behind) and every byte must be consumed by one path on one vCPU. The two bugs it exists for -- a transition applied after the UART's lock was dropped; "the line is up" returned for the run loop to act on -- have windows too narrow to hit on purpose, so their bug-proofs widen them; this is the regression test that the device decides and raises under one lock |
...
| `test_fdt` (host) | -- | the writer's blob has a sound header and is the size it says; every node and property the machine description promises reads back through the reader, each equal to the uapi constant it came from; a truncated blob, a blob that does not fit, a node left open and a property outside any node are refused |
...
`guest_uart_wfi`, `guest_uart_poll` and `guest_dtb` (the first written in
C, with an arm64 Image header, linked at `COSMO_HVM_RAM_BASE + text_offset`
and loaded there) -- one per exit the EL2 switch decodes, then one per
