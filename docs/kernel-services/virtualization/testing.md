...
| Linux boot (userland, optional) | a raw arm64 `Image` at `tests/hv/aarch64/Image` (gitignored) via `vmctl run --machine` in `rc.test` | a stock Linux kernel, when a developer has provided one: `vmctl` builds the device tree, loads the Image by its header, answers PSCI and runs it; a diskless kernel comes up and panics on the root filesystem it cannot find. The boot harness (`run_boot_test.py`) requires the milestone -- `Booting Linux`, `Machine model: cosmo,virt` (our device tree's compatible, read back by Linux), and `Unable to mount root fs` (a *late* string, so an early banner then a silent hang would not pass) -- only when `rc.test` printed `LINUX: present`, so a tree with no Image stays green. There is no kernel self-test for this: a Linux boot is seconds long and would blow the harness's 8 s per-test budget, so it lives in the userland run where nothing budgets it |
