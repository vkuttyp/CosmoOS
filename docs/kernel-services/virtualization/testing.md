...
| `el2-virtq-device` | `aarch64/guest_vblk.c` | the virtio-blk mechanism end to end with a real guest driver: the test models the virtio-mmio block transport inline (MagicValue/Version/DeviceID, status handshake, feature negotiation, the queue the guest sets up) over an in-test disk of known bytes; the guest probes the node, negotiates, builds a ring, reads sector 1 and polls the used ring; the test requires the guest to read back exactly the disk's bytes, so a device that completed the request without serving it would not pass |
| `test_vblk_dev` (host) | -- | the device-side virtqueue walk (`vblk.c`) with no guest and no QEMU: an in-memory guest RAM holds a ring a driver would build and an in-memory disk holds per-sector bytes; a read returns the requested sector's bytes (a per-sector pattern, so serving the wrong sector fails the compare) and completes on the used ring; a write (`VIRTIO_BLK_T_OUT`) completes as `UNSUPP`; and five hostile rings are refused with no read out of bounds -- a head index past the ring (large enough to leave guest RAM, so the bound is the only thing that stops it), a chain that loops, a data buffer outside guest RAM, a read past the disk (an I/O error), and a queue never marked ready or sized absurdly |
