...
| brk, mmap | `brk(0)`, grow 8 KiB and write both pages, shrink back, a request below the start returns the old break; anonymous `mmap` of 8 KiB written, `mprotect` to read-only, `munmap`; a file mapping `-ENODEV`; RWX `-EINVAL`; `MAP_FIXED` at a free page, then `munmap` |
