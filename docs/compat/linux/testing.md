...
`-EINVAL`; an `AF_INET6` address with `::1`; `AF_UNIX` is a family since
the unix-sockets unit (its rows are below).
...
| named pipes | (the named-pipes unit) `mknodat(AT_FDCWD, path, S_IFIFO\|0644)` 0, `S_IFCHR` `-1` (`EPERM`), `S_IFSOCK` `-22`, the legacy `mknod` of the same path `-17` (x86-64); `openat(O_WRONLY\|O_NONBLOCK)` `-6` with no reader; a non-blocking reader then a non-blocking writer; `read` `-11` while empty; 4 bytes across; `fstat` `S_IFIFO`; `F_GETFL` shows `O_NONBLOCK`, `F_SETFL(0)` clears it, `F_SETFL(O_NONBLOCK)` and `read` `-11` again; the writer closed: `read` 0; `unlinkat` |
...
| file mmap (milestone 10; the file-regions unit) | no mapping type, or a type past `MAP_SHARED_VALIDATE`, is `-EINVAL` (anonymous too) and type 3 is shared; a 6000-byte file mapped `PROT_READ` over 8 KiB matches and its tail is zero; a `PROT_READ\|PROT_WRITE` mapping at offset one page matches and a write to it leaves the file unchanged (`pread64`: copy-on-write); `MAP_SHARED\|PROT_WRITE` succeeds -- a byte written through it is read by `pread64` at once, a `pwrite64` is seen in it at once, `msync(MS_SYNC)` 0, `MS_SYNC\|MS_ASYNC` and an undefined bit `-EINVAL` (this used to assert `-EOPNOTSUPP`); a read-only `MAP_SHARED` sees a later `pwrite64`; a read-only fd: `MAP_SHARED\|PROT_WRITE` `-EACCES`, and `mprotect(PROT_WRITE)` of its shared mapping `-EACCES`; an unaligned offset `-EINVAL`; a bad fd `-EBADF` |
...
| select, pselect6 | (the device-readiness unit) a pipe pair: `pselect6` with a zero timeout returns 1, the write end's bit set, the read end's and the except set's clear; `nfds` 1025 `-EINVAL`; a closed fd's bit `-EBADF`, and the same bit above `nfds` not looked at (0); a pipe's write end with its reader gone is not in the except set (0) and is in the writable set (1); a 20 ms timeout with nothing ready returns 0 after at least 15 ms; a byte written: readable; a mask admitting a pending `SIGUSR1` runs the handler, `-EINTR`, the old mask back; `select` (x86-64) with a timeval, a timeval with `tv_usec` past a second `-EINVAL`; the writer closed: readable; **a `select` over `/dev/net/tap` and a UDP socket**: neither ready, eight ARP requests written, the tap's bit alone set within 2 s and the reply read; the bench `LINUXBENCH: pselect6 … ppoll …` |
| futex | `FUTEX_WAIT\|PRIVATE` with the wrong value `-EAGAIN`; `FUTEX_WAIT` with a 20 ms timeout `-ETIMEDOUT`; `FUTEX_WAKE` returns 0; operation 99 `-ENOSYS`; a misaligned word is `-EINVAL` on the past-deadline `WAIT_BITSET` path and on `WAKE` (the fast path used to read the word and answer `-EAGAIN`/`-ETIMEDOUT`); the shared-futex unit: on a `MAP_SHARED` page, a clone thread waiting *with* `FUTEX_PRIVATE_FLAG` (counted asleep through a self `CMP_REQUEUE` with the flag) is woken 0 by a `WAKE` without the flag and 1 by one with it, and the reverse pair for a waiter without -- the flag selects the key and both keys work |
| unix sockets | (the unix-sockets unit) `socket(AF_UNIX, SOCK_SEQPACKET)` `-94`; a path bound with a NUL-terminated `sockaddr_un` and connected with an unterminated one, a write before the accept, `accept4`'s peer of length 2, `getpeername` the path, `SO_PEERCRED` this pid, `openat` of the node `-6`; `sendmsg` with `SCM_RIGHTS` carrying a pipe's write end and `recvmsg` receiving a descriptor that writes into the pipe (`msg_controllen` 24, `cmsg_len` 20); the same with 8 bytes of control room: the bytes arrive, `MSG_CTRUNC`, `msg_controllen` 0; the client closed, the server reads 0; an abstract name bound, connected and read back from `getpeername`, `accept4(SOCK_NONBLOCK)` inherited; `socketpair`; a datagram by name with a nameless sender |
| random, sockets | `getrandom(32)` returns 32; a UDP socket bound to 127.0.0.1 sends `ping` to itself and `recvfrom` receives it with the sender's address; `getsockname` into a 4-byte buffer reports 16 and leaves a canary intact; `fstat` on the socket is `S_IFSOCK`; `setsockopt(SOL_SOCKET, SO_REUSEADDR)` 0; `close`; `socket(AF_UNIX)` accepted since the unix-sockets unit (the row above) |
...
(answered since the terminal-modes unit, so the output is line-buffered
