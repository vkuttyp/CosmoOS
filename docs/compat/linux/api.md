...
| 2 | `open` | `lx_open_flags` → `vfs_open(cwd, path, flags, mode & 07777)`; handle rights from the access mode, plus `READ|WRITE` for a directory, which `*at` calls through it need (P31) | `O_NONBLOCK` reaches the kernel since the named-pipes unit (a FIFO's open rules and per-open mode; a regular file ignores it); `O_CLOEXEC O_NOCTTY O_LARGEFILE` accepted and dropped; any other unknown flag `-EINVAL` |
| 85 | `creat` | `open(O_WRONLY|O_CREAT|O_TRUNC)` | |
| 257 | `openat` | as `open`, from the directory `at_base` resolves (the dirfd unit): `AT_FDCWD` or an absolute path gives the cwd, any other descriptor must be an open directory; a directory it opens records the name the walk took to it (P32), for `fchdir` | `-EBADF` for no such handle or one without the right the call needs -- `READ` to look a name up, `WRITE` to change an entry (a creating, truncating or write-mode `openat` included; both directories of a `renameat`) -- and `-ENOTDIR` for one that is not a directory. A directory opened at either door carries both rights, so a program may open it read-only and change entries through it, as on Linux; a delegation that narrows the rights keeps them narrowed, and the handle `openat` returns is masked by the descriptor's rights (`at_base`'s `have`), so a child directory opened through a `READ`-only one is `READ`-only too (P31) |
...
| 83, 258 | `mkdir`, `mkdirat` | `vfs_mkdir(start, path, mode & 07777)`, `start` the cwd for `mkdir` and `at_base`'s directory for `mkdirat` | `mkdirat` needs `WRITE` on its descriptor |
| 133 / —, 259 / 33 | `mknod`, `mknodat` | (the named-pipes unit) `S_IFIFO` → `vfs_mknod(start, path, mode & 07777, VNODE_FIFO)`, `start` the cwd for `mknod` and `at_base`'s directory (needing `WRITE`) for `mknodat`; `mknod` is x86-64's legacy form (AArch64 has only `mknodat`) | `S_IFSOCK` `-EINVAL` (a socket's name is made by `bind`); any other type `-EPERM`, Linux's answer to a caller without `CAP_MKNOD`; `dev` ignored; `mknodat` from `at_base` |
| 84 | `rmdir` | `vfs_rmdir` | |
...
| 88, 266 | `symlink`, `symlinkat` | `vfs_symlink(start, path, target)`, `start` the cwd for `symlink` and `at_base`'s directory (needing `WRITE`) for `symlinkat` | the target is stored verbatim and resolved when the link is followed; `-EPERM` on a filesystem without links, `-EOPNOTSUPP` on a cosmofs older than format version 8 |
...
| 80 | `chdir` | `process_chdir`: publishes the path the walk took (P32), so through a symbolic link `getcwd` answers the directory's own path, and `..` after it that directory's parent | |
| 81 / 50 | `fchdir` | (the dirfd unit) `process_fchdir(f->vn, f->dir_path)`: the descriptor's directory and the name its file recorded at open, published together as `chdir` publishes them | `-EBADF`, `-ENOTDIR` (checked before the name), `-ENOENT` for a directory file with no recorded name rather than an invented one (since the cwd-name unit a directory opened through a symbolic link has one: its own path, P32); the name goes stale on a rename of the directory or any ancestor, as `chdir`'s does |
...
| 9 | `mmap` | `lx_prot`; `MAP_FIXED` → the replacement (`vm_user_map_anon_replace`, or `vm_user_map_file` with `VM_MAP_REPLACE` for a file: the range owned throughout, `docs/audit/next-subsystem-map-fixed.md`) at `addr` (must be page aligned and in range, else `-EINVAL`); otherwise `vm_user_find_free` from a page-aligned hint, falling back to `USER_MMAP_BASE`. A file (since the file-regions unit, `docs/kernel/memory/design.md` §7): a `VM_REGION_FILE` region over the file's page cache, demand-paged -- `MAP_SHARED` the file's own pages, `MAP_PRIVATE` copy-on-write; the fd needs READ (`-EBADF`), a regular file (`-ENODEV`), the offset page aligned (`-EINVAL`); `MAP_SHARED|PROT_WRITE` needs the file opened for writing through a handle with WRITE (`-EACCES`; one `handle_get` lookup carries the file and its rights, so a concurrent close and reopen of the number cannot split the decision from the file), and such a mapping of a read-only fd cannot later be `mprotect`ed writable (`-EACCES`); a touch past the end of the file is `SIGBUS`. This row used to describe an eager copy through a bounce page and `-EOPNOTSUPP` for a shared writable mapping; both are gone | the mapping type (the low four bits) must be `MAP_SHARED` 1, `MAP_PRIVATE` 2 or `MAP_SHARED_VALIDATE` 3 (shared), anonymous or not: none, or any other value, `-EINVAL` (review found this door deciding on the SHARED bit alone); `PROT_WRITE|PROT_EXEC` `-EINVAL` (W^X); `MAP_NORESERVE`, `MAP_STACK`, `MAP_POPULATE` ignored; `MAP_SHARED|MAP_ANONYMOUS` is private to the process (unobservable without `fork`); `len` 0 or larger than the user window `-EINVAL`; no free range `-ENOMEM` |
...
| Nr | Call | Translation | Deviations |
|---|---|---|---|
| 41 | `socket` | `ksock_create` for `AF_UNIX` (1, since the unix-sockets unit; `SOCK_SEQPACKET` is `-ESOCKTNOSUPPORT`), `AF_INET` (2)/`AF_INET6` (10), `SOCK_STREAM` (1)/`SOCK_DGRAM` (2); handle with READ and WRITE | `SOCK_NONBLOCK`/`SOCK_CLOEXEC` dropped; other families `-EAFNOSUPPORT`; a known type the family lacks (`SEQPACKET`, `RAW`, `RDM`, `DCCP`, `PACKET`) `-ESOCKTNOSUPPORT`, a type that is no type `-EINVAL`; protocol ignored |
