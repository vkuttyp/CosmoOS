...
| 257 | `openat` | as `open`, from the directory `at_base` resolves (the dirfd unit): `AT_FDCWD` or an absolute path gives the cwd, any other descriptor must be an open directory; a directory it opens records its normalised path, for `fchdir` | `-EBADF` for no such handle or one without the right the call needs -- `READ` to look a name up, `WRITE` to change an entry (a creating `openat` included; both directories of a `renameat`) -- and `-ENOTDIR` for one that is not a directory. A directory opened at either door carries both rights, so a program may open it read-only and change entries through it, as on Linux; a delegation that narrows the rights keeps them narrowed (P31) |
