...
| **Taken up by `docs/audit/next-subsystem-twelve-bytes.md`** (not struck until it lands, and not necessarily then). **`net-harness`: localised by PR #167, still open, and now a much smaller target.** The harness was made to record its timings, and its own CI job answered within the hour: `ready at 90.9s, back-connection accepted at 92.0s, ... gave up 102.0s`. **The accept succeeded**; the `TimeoutError` ten seconds later is `conn.settimeout(10)` on the *accepted* connection, so it came from `recv`, not `accept`. The deadline PR #167 fixed (120 s armed in `NetTest.__init__`, before QEMU launches at `run_boot_test.py:589`) was a real defect and was **not** the cause -- which is why that unit declined to claim it. What is left: the TCP connection is established both ways (`ksock_connect` returns 0, the host accepts), the guest's **twelve bytes never arrive** within ten seconds, and everything else on the same interface in the same run is fine (`NETTEST: done tcp_conns=2 udp_pkts=20 quit=1` -- a 256 KiB TCP echo and twenty UDP datagrams). The guest spent 11.6 s against an 8 s budget, consistent with sending and then blocking in `ksock_recvfrom`. So: **twelve bytes, guest to host, on a connection both ends agree exists** -- either the guest's TCP send path or slirp's guest-to-host delivery. **It reproduces on this developer's machine on x86-64, rarely -- one run in twenty-one**, and six of those boots ran under CPU load without raising it (earlier notes said one in three and then one in fifteen; the first was one failure in three runs stated as a rate), with the signature identical to CI's (`accepted at 76.1s, then 0 of 12 bytes: b'', gave up at 86.1s, accept budget 78.7s unspent`) -- so the deadline is excluded rather than merely unproven, and the loop is two minutes instead of a CI round trip. Every earlier local attempt was on **aarch64**, where it did not reproduce in eleven runs; it appeared on the first x86-64 attempt, which is probably why the row said for weeks that it rarely reproduced locally. **The bug itself is on both architectures** -- CI failed it on x86-64 (`accepted at 92.0s, 59.1s unspent`) and aarch64 (`accepted at 92.6s, 65.3s unspent`) within the hour, same signature -- so the architecture decides where the loop runs, not what the defect is. That is the next unit | none (found by the hardening unit; localised by the nettest-deadline unit) |
