...
| **`cosmofs-writeback` can fail on a loaded machine, and the test is right**: `cfs_writeback_thread` bumps `fs->wb_commits` *after* `cosmofs_sync` returns, i.e. after the commit that advanced `fs->sb.generation` has already released `fs->lock`. `cosmofs_stats` reads both fields under that lock, so a reader can observe the new generation with the old count -- exactly the pair the test asserts (`st1.generation == st0.generation + 1 && st1.wb_commits == st0.wb_commits + 1`). The increment is also an unlocked read-modify-write on a field read under the lock elsewhere. Seen once, aarch64 CI, PR #162: it failed after **80 ms** rather than at the 2 s deadline, with `committed generation 2` in the log -- the commit happened and the counter had not caught up. **Pre-existing**: `git log -S` puts the line in 9171b8d (the transaction engine, PR #22), untouched since; PR #160 changed when the thread is *created*, not what it does after a commit. The fix is to publish the counter in the same critical section that publishes the generation | none (found by the VMState-layout unit) |
