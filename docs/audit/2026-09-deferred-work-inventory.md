...
| AArch64 hardening: ~~`SCTLR_EL1.WXN` cleared and never set~~ (set on every CPU from the kernel's tables on, proved by `make test-wxn`, PR #140); UAO, E0PD, BTI, PAC unused; **a user-triggerable SError panics the kernel -- taken up by `docs/audit/next-subsystem-async-error.md`** (not struck until it lands; the unit takes this clause only, and x86-64's `#MC` turns out to have the same gap: every SError reaches `aarch64_trap_entry`'s `default` arm and every machine check is an unregistered vector, so both panic even when the hardware says it **corrected** the error. The unit does NOT kill the interrupted process on a contained error -- review refused that: an asynchronous abort's frame names the context interrupted at delivery, not the one that caused it, so attribution needs the RAS error records and is the unit after); no device-tree parsing for the host (ACPI only); PSCI variations untested | 13.2, 13.3 |
