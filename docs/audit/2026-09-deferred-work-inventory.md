...
| **`SYS_mprotect` for native programs** — the Linux personality already has it (`lx_mprotect`, `compat/linux/syscalls.c:1012`) over a `vm_user_protect` that is built, tested and used by the ELF loader; only the native ABI has no call. **Taken up by `docs/audit/next-subsystem-mprotect.md`**, which also records that `MAP_FIXED` replacement cannot substitute for it — replacement gives back demand-zero memory, and a protection change has to keep the contents | README.md:1486 |
