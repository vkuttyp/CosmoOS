...
| ~~**native futex requeue**~~ **BUILT (native thread door unit)**: `SYS_futex_requeue` 94, the compare form, and `cosmo_cond_broadcast` requeueing under invariant L10 (the herd measured: 1 sleep on the mutex word against 7). The original entry read: `futex_requeue` is built (`kernel/ipc/futex.c:159`) and the Linux door calls it; `cosmo_cond_broadcast` wakes every waiter and says the herd is what requeue exists to avoid. Taken up by `docs/audit/next-subsystem-native-thread-door.md`, with per-thread signal targeting, as one unit | README.md:1486 (the Linux personality has requeue since milestone 10) |
