...
| ~~`chdir` through a symbolic link publishes the link's spelling with the target's vnode, at both doors: `getcwd` then names the link rather than the directory, and a later relative `chdir("..")` normalises lexically against that spelling and can publish a name unrelated to the vnode it reaches.~~ **BUILT (the cwd-name unit, `docs/audit/next-subsystem-cwd-name.md`, invariant P32)**: `chdir`, a spawn's `cwd` and a directory file's name are the path the walk took (`vfs_lookup_named`); `fchdir` of a directory opened through a link succeeds with its own path; procfs's process directories answer `..` and `/proc/self` is a link. What follows is the row as it stood. The dirfd unit made `fchdir` refuse a directory with no coherent name (a recorded name is walked again with no link allowed and must reach the same vnode); `chdir` keeps its lexical ("logical") name because refusing it would break every shell `cd` into a linked directory, and the coherent answer -- the resolved path, which needs either a dentry cache or parent pointers -- is its own unit. **Taken up by `docs/audit/next-subsystem-cwd-name.md`**, which measured it at both doors (`tools/chdir-link-probe.py`: after `chdir` through a link and `chdir("..")`, `getcwd` names `/tmp` while the process stands in `/tmp/clp`) and found a cheaper answer than either: the walk already knows which components it entered, so it keeps that name (P32). The same probe found `chdir("..")` from `/proc/<pid>` is `ENOENT` -- procfs's process directories do not answer `..` -- which the unit also closes | none (found in review of the dirfd unit) |
