...
| **Built as PR #167 and still open** -- deliberately. The unit fixed the deadline and made the harness report its timings; it did **not** establish that the deadline was the cause, so the row is struck only when a sighting carrying the new timings shows it was and the fix ended it. Measured while building: the back-connection lands at 72% of the boot (76-83 s here), projecting onto CI's 140-146 s boots at 101-105 s against the old 120 s deadline -- a margin of 15-19 s, thin but not shown to be crossed, since a 145.6 s boot passed and a 145.8 s one failed. `net-harness` fails intermittently on both architectures, in CI and on this developer's machine -- twice in the eight runs of PR #140, again in #142, twice in #144, again in #146, twice on documentation-only commits, and **seven times in the fortnight to 2026-09-17**, four of them consecutively on one branch about a lock in cosmofs (after which the *identical commit* passed on a re-run, with 5/5 locally) and the seventh on the report's own documentation-only pull request. The report finds a **verified structural defect**: `tests/boot/nettest.py:36` sets a hard-coded 120 s `accept()` timeout and starts the accepting thread in `__init__`, which `run_boot_test.py:536` runs *before* QEMU launches at `:589`; it is shorter than the harness's own sibling deadline for the same event (`args.timeout - 30`, 150 s, started after the launch); and on expiry the handler closes the listener. **It does not show that this is the cause**: across sixteen recent aarch64 jobs a 145.6 s boot passed and a 145.8 s one failed, so boot duration does not predict the outcome. What is established is that the harness cannot tell the difference -- it reports `TimeoutError` and nothing about when the guest connected or how much budget was left -- and that is the defect the unit fixes first. This row's "not a timing bound" and "needs the host side instrumented" are both superseded: the error is already printed; the *timing* is not | none (found by the hardening unit, whose extra runs made it visible) |
