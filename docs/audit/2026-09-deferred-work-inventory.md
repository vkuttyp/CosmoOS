...
| **Taken up by `docs/audit/next-subsystem-nettest-deadline.md`** (not struck until it lands), which **corrects this row's diagnosis**. `net-harness` fails intermittently on both architectures, in CI and on this developer's machine -- twice in the eight runs of PR #140, again in #142, twice in #144, again in #146, twice on documentation-only commits, and six times in the fortnight to 2026-09-17, four of them consecutively on one branch whose subject is a lock in cosmofs, after which the *identical commit* passed on a re-run while the same build passed five of five locally. The observations this row collected are right and the conclusion drawn from them was not: **it is not host-dependence and it is not unbounded.** `tests/boot/nettest.py:36` sets a hard-coded 120 s `accept()` timeout and starts the accepting thread in `__init__`, which `run_boot_test.py:536` runs *before* QEMU launches at `:589`; on expiry the handler closes the listener. The guest prints `NETTEST: ready` at t+82 s on a machine that passes 5/5 and connects immediately after, while the failing CI boots took 132-146 s. `ksock_connect` returns 0 because QEMU's user networking completes the guest's handshake locally and only then connects to a port that has closed, and the `TimeoutError` this row wanted the host instrumented for is `accept()` giving up -- `failures()` has been printing it in every failing log all along. The harness's sibling deadline for the same event is `args.timeout - 30` (150 s), derived and started with the run; the accept one is shorter *and* earlier | none (found by the hardening unit, whose extra runs made it visible) |
