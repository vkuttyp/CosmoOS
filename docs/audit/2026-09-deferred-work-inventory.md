...
| **Taken up by `docs/audit/next-subsystem-cwd-hold.md`** (not struck until it lands). The cwd-ref fix is a regression test, not a proof; the seam that would prove it is named and not built. Measured by `tools/cwd-race-probe.py`: with the reference removed at the native `open` and every freed vnode poisoned, the test catches it in one of five x86-64 boots and none of three AArch64 -- intermittently, which is a rate and not a proof; today's tree, with nothing poisoned, cannot see it at all | README.md:1637; `docs/audit/next-subsystem-cwd-ref.md` |
