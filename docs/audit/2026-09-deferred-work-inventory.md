...
| **no machine-wide time source: a deadline built on one CPU and tested on another is unsound when the counter is not common.** `clock_deadline_ns`/`clock_deadline_passed` centralise the hazard and saturate, but measure against the clock. A source two CPUs can compare without trusting their counters needs a designated timekeeper with handoff when it goes offline or stops answering. Two shapes are known not to work, both tried: the maximum of per-CPU tick counts stalls, because those counters do not share an origin and the leader stopping blocks every follower for the whole of bring-up; and every CPU contributing its own delta advances at `CONFIG_HZ` times the CPU count, so deadlines expire that many times too early (`docs/audit/next-subsystem-cpu-clock.md`) | 6.4 |
