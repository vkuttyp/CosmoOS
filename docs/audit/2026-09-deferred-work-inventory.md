...
| **Taken up by `docs/audit/next-subsystem-socket-verdict.md`** (not struck until it lands). ICMP errors for a UDP flow in the host's reply state ("no consumer exists yet") -- the consumer is the unit: `sock_set_error` has zero callers, `struct socket::error` is never written, and `icmp_input` (`ipv4.c:374`) handles only fragmentation-needed, echo and echo-reply, so this host sends ICMP port-unreachable (`udp.c:290`) and has never received one | README.md:1342 |
