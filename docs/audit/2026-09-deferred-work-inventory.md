...
| Two of `cosmofs_check`'s ten finding classes have no test that manufactures them: `chain_cycle` (a cycle needs a corruption hook that writes structure, not one that flips a field, so the `CFS_CHECK_MAX_CHAIN` bound of 4096 is exercised by nothing) and three of `dir_bad`'s four sites (an over-long `namelen`, a directory with two parents, a pointer outside the pool). They are reported by code no test has fired | 8.5 (named by the fsck unit) |
| No operator interface for the filesystem's maintenance passes: `cosmofs_scrub` and `cosmofs_check` are kernel-facing, and reaching them needs a name for a mount (procfs is a per-process hierarchy with no mounts directory), a stable identity across mount namespaces, and a rule that a file held open pins its target mount | 8.5 (named by the fsck unit) |
...
| `net-harness` fails intermittently in CI and never locally: twice in the eight runs of PR #140 and again in PR #142, on both architectures, twice on documentation-only commits. The guest's `ksock_connect` to the gateway returns 0 and `NETTEST: client failed (0)` follows, while the host harness's `accept` ends in `TimeoutError` -- so the guest reached slirp and slirp never delivered the connection to the listener on `127.0.0.1`. It is not a timing bound, so `docs/testing/flakes.md`'s rule keeps it off that list; what it needs is the host side instrumented (slirp's own error, the port's state) on a run that fails, which needs a CI run that fails on purpose | none (found by the hardening unit, whose extra boots per run raised the exposure) |
