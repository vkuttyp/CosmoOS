...
| The loader is tested against two EDK2 builds and two CPU models by hand, not by CI: CI runs Debian's AAVMF only, and the VHE handover path (`HCR_EL2.E2H` set at hand-over, PR #140) exists because that build differs from the one a developer has locally. A firmware matrix in CI, or a documented minimum EDK2, is unbuilt | 13.3 (found by the hardening unit) |
| The IOMMU walker (`kernel/iommu/pt.c`) builds one four-level root, a level-0 start the architecture allows only above 42 bits of output; an SMMU of 42 bits or less is refused at probe rather than programmed (PR #140), and the concatenated level-1 root it would need is unbuilt -- untestable on QEMU's 44-bit SMMU; the hypervisor's stage-2 (`hv_s2_layout`) has the rule and the shape | 13.2 (found by the hardening unit) |
