...
2. **A grace-period primitive: `quiesce`.** Epoch-based reclamation with the quiescent state defined as "CPU passed through the IRQ-return path with `irq_depth == 0` or through `schedule()`", both already instrumented. API:
   - `void quiesce_read_lock(void)` / `quiesce_read_unlock(void)`: mark a read-side section; in this kernel they are `preempt_disable()`/`preempt_enable()` plus, in debug builds, a per-CPU depth counter for assertions. Interrupt handlers are implicitly read-side (IRQs off).
   - `void synchronize_quiesce(void)`: sleeping; waits until every online CPU has passed a quiescent state since the call began. Implementation: bump a global epoch, then wait for each CPU's per-CPU `last_seen_epoch` (updated at the two quiescent points, one relaxed store) to reach it; idle CPUs are counted as quiescent immediately (they are in `hlt`/`wfi` with the idle marker set). Bounded by one tick (4 ms) per waiting round; no IPIs in the common case.
