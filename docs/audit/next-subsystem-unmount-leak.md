...
| `cosmofs-freelog-idempotent` | mount, replay, unmount without committing, mount again: the same blocks, the same count, and `free_root` unchanged, because a record is retired by a commit and not by a read | clear the record at replay (`replay-clears-record`): the second mount finds nothing and the blocks are stranded again |
