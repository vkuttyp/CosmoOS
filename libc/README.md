...
`crt0.S`, `errno` and the thread block behind it (`tcb.c`), strings, the
allocator, stdio and `printf`, the
system-call wrappers, directories, `spawnve`/`spawnvp`/`waitpid`/`kill`,
sockets with `inet_pton`/`inet_ntop`. `libc.mk` builds `libc.a` and
`crt0.o`; every program links against them. The allocator and stdio are
locked for threaded programs, and `errno` is per-thread: it lives in a
block behind the thread pointer that `SYS_set_tls` installs, one for the
first thread and one beside each created thread's stack
(`docs/libc/invariants.md` L8). No
